    ...
    class Program
    {
        static void Main(string[] args)
        {
            using (var db = new MovieContext())
            {
                ...
                Console.WriteLine();
                Console.WriteLine("Elenco de Star Wars (press any key...):");
                Console.ReadKey();
                var elencoStarWars = from c in db.Characters
                ...
                foreach (var ator in elencoStarWars)
                {
                    Console.WriteLine("\t{0}\t{1}", ator.Personagem, ator.Nome);
                }

                Console.WriteLine();
                Console.WriteLine("Todos os agentes 007 e seus filmes (press any key...):");
                Console.ReadKey();
                var atoresJamesBond = db.Characters.Where(c => c.Character == "James Bond").Select(c => c.Actor).GroupBy(c => c);
                var filmesAtoresJamesBond = db.Characters.Where(c => c.Character == "James Bond").Select(c => c).GroupBy(c => new { c.Actor, c.Movie });
                foreach (var ator in atoresJamesBond)
                {
                    Console.WriteLine("\t{0}", ator.Key.Name);
                    foreach (var filmeAtor in filmesAtoresJamesBond)
                    {
                        if (filmeAtor.Key.Actor.Name == ator.Key.Name)
                        {
                            Console.WriteLine("\t\t{0}", filmeAtor.Key.Movie.Title);
                        }
                    }
                }

                Console.WriteLine();
                Console.WriteLine("Qual ator desempenhou mais vezes o personagem James Bond (press any key...):");
                Console.ReadKey();
                var atorJamesBond = db.Characters.Where(c => c.Character == "James Bond").Select(c => c.Actor).GroupBy(a => a).OrderByDescending(a => a.Count()).FirstOrDefault();
                Console.WriteLine("\t{0}", atorJamesBond.Key.Name);


                Console.WriteLine();
                Console.WriteLine("Todos os filmes que os atores que interpretaram James Bond fizeram (press any key...):");
                Console.ReadKey();
                var filmes = from c in db.Characters
                             where c.Character == "James Bond"
                             group c by c.Actor into actors
                             select new
                             {
                                 Nome = actors.Key,
                                 Filmes = from c in db.Characters
                                          where c.Actor == actors.Key
                                          select c.Movie.Title
                             };

                foreach (var agente in filmes)
                {
                    Console.WriteLine("\t{0}", agente.Nome.Name);
                    foreach (var filme in agente.Filmes)
                    {
        ...
        private static void Seed(MovieContext context)
        {
            ...
            var actorCharacters = new List<ActorMovie>() {
                ...
                #endregion
                #region ator ficticio para exercicio
                    new ActorMovie { ActorId = actors.Single(g => g.Name == "Daniel Craig").ActorId, Character = "Joe Van Hollow", MovieId = movies.Single(g => g.Title == "Forrest Gump").ID },
                #endregion
            };
